...
class ReaderFrame(object):
    class Sync(object):
        def __init__(self, tari, rtcal, delim=12.5e-6):
            self.delim = delim
            self.tari = tari
            self.rtcal = rtcal
    ...
    class Preamble(Sync):
        def __init__(self, tari, rtcal=None, trcal=None, delim=12.5e-6):
    ...
    def duration(self):
        ...
        if self.preamble is None:
            raise AttributeError('ReaderFrame is missing "preamble"')
...
class TagFrame(object):
    ...
    def preamble(self):
        if self.m is None:
            raise AttributeError('TagFrame missing "m"')
        if self.trext is None:
            raise AttributeError('TagFrame missing "trext"')
